...
public partial class RedisCacheManager
{
    ...
    public void DeleteMail(int characterid, int mailid)
    {
        ...
        //string sql = string.Format("DELETE FROM mail WHERE id = {0}", mailid);
...
partial class CacheManager
{
    ...
    //发送邮件
    public void SendMail(int characterid, MailData _mail)
    {
        string sql = string.Format("INSERT INTO mail (sender_id,receiver_id,subject,body,deliver_time,money,has_items) VALUES ({0},{1},'{2}','{3}','{4}',{5},{6})", _mail.sender_id, _mail.receiver_id, _mail.subject, _mail.body, _mail.deliver_time, _mail.money, _mail.has_items);
        MysqlManager.instance.ExecNonQuery(sql);
    ...
    public void WriteMailData(int characterid)
    {
        ...
        // 删除数据库中的邮件数据
        string sql = string.Format("DELETE FROM mail WHERE receiver_id = {0}", characterid);
        MysqlManager.instance.ExecNonQuery(sql);

        // 将缓存中的数据插入到数据库中
        for (int i = 0; i < mails.Count; i++)
        {
            MailData _mail = mails[i];
            sql = string.Format("INSERT INTO mail (sender_id,receiver_id,subject,body,deliver_time,money,has_items) VALUES ({0},{1},'{2}','{3}','{4}',{5},{6})", _mail.sender_id,_mail.receiver_id, _mail.subject, _mail.body, _mail.deliver_time, _mail.money, _mail.has_items);
            MysqlManager.instance.ExecNonQuery(sql);
        }
    }
