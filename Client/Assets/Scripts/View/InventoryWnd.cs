...
public class InventoryWnd : BaseWnd
{
    ...
    {
        _invContent = _transform.FindChild("Scroll View/Viewport/Content");

        Button btnReturn = _transform.FindChild("BtnClose").GetComponent<Button>();
        btnReturn.onClick.AddListener(OnReturn);

        Button btnItem = _transform.FindChild("Scroll View/Viewport/BtnItem").GetComponent<Button>();
        ...
        foreach (InventoryDTO item in invs)
        {
            if (item.itemid > 0)
            {
                ...
                child.FindChild("ItemID").GetComponent<Text>().text = item.itemid.ToString();

                ItemCfg cfg = ConfigManager.instance.GetItemCfg(item.itemid);
                Image img = child.FindChild("Image").GetComponent<Image>();
        ...
        _equipContent = _transform.FindChild("EquipContent");
    ...
    public void UpdateInv(InventoryDTO inv)
    {
        ...
        if (inv.itemid < 0)
        {
            ...
            img.overrideSprite = null;
            img.color = EmptySlotColor;
            child.FindChild("ItemID").GetComponent<Text>().text = string.Empty;
